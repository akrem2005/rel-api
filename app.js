...
app.get("/api/properties", async (req, res) => {
    let query = "SELECT p.*, u.name as ownerName FROM properties p JOIN users u ON p.ownerId = u.id WHERE 1=1";
    const params = [];
    const filters = [
        "category",
        "listingType",
    ...
    filters.forEach((f) => {
        if (req.query[f]) {
            ...
            else if (f === "search") query += " AND (p.name LIKE ? OR p.location LIKE ?)";
            else query += ` AND p.${f} = ?`;
    ...
    async (req, res) => {
        try {
            ...
            const {
                ...
                floorNumber, parkingSpaces, maxGuests, existingImages
            } = req.body;
            // 1. Handle Images
            let finalImages = [];
            ...
            await pool.query(
             ...
             WHERE id=?`,
                [
                    name, location, price, description, category, listingType,
                    bedrooms || null, bathrooms || null, area || null, furnishingStatus || null,
                    floorNumber || null, parkingSpaces || null, maxGuests || null, imagesJson,
                    propertyId
                ]
            );
